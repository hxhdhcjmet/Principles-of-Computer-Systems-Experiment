...
/* ========== SIMD SM4 engine (adapted from sm4_simd_sbox.c) ========== */

#define SM4_BLOCK_SIZE 16
#define SM4_NUM_ROUNDS 32
#define SM4_NUM_STREAMS 8
...
/* ---------- BOX tables (precomputed T-table for bit-sliced lookup) ---------- */
static const uint32_t BOX0[256] = {
...
static const uint32_t BOX1[256] = {
...
static const uint32_t BOX2[256] = {
...
static const uint32_t BOX3[256] = {
...
/* ---------- SIMD SM4 key schedule ---------- */
static void sm4_simd_key_init(const uint8_t key[16], uint32_t rk[32])
{
    uint32_t k[4];
    uint32_t tmp;
    ...
    for (int i = 0; i < 32; i++) {
        ...
        rk[i] = k[0] ^ tmp ^ rotl32(tmp, 13) ^ rotl32(tmp, 23);
        k[0] = k[1];
        k[1] = k[2];
        k[2] = k[3];
        k[3] = rk[i];
...
/* ---------- SIMD x8 encrypt / decrypt (AVX2 bit-sliced) ---------- */
...
/* Process 8 × 16-byte blocks (128 bytes total) in one call.
 * in/out layout: block0[16], block1[16], ..., block7[16] */
static void sm4_simd_do(const uint8_t *in, uint8_t *out,
                        const uint32_t rk[32], int enc)
{
    __m256i X[4], Temp[4], Mask;
    Mask = _mm256_set1_epi32(0xFF);

    /* Load 8 blocks (128 bytes) as 4 __m256i values */
    ...
    /* Bit-slice: pack same word-position from all 8 blocks */
    ...
    __m256i vindex =
        _mm256_setr_epi8(3, 2, 1, 0, 7, 6, 5, 4,
    ...
    /* 32 rounds */
    for (int i = 0; i < 32; i++) {
        __m256i k = _mm256_set1_epi32((enc == 0) ? rk[i] : rk[31 - i]);

        Temp[0] = _mm256_xor_si256(
            _mm256_xor_si256(X[1], X[2]),
            _mm256_xor_si256(X[3], k));

        /* S-box + linear via 4-table gather lookup */
        Temp[1] = _mm256_xor_si256(
            X[0],
            _mm256_i32gather_epi32((const int *)BOX0,
                                   _mm256_and_si256(Temp[0], Mask), 4));
        Temp[0] = _mm256_srli_epi32(Temp[0], 8);
        Temp[1] = _mm256_xor_si256(
            Temp[1],
            _mm256_i32gather_epi32((const int *)BOX1,
                                   _mm256_and_si256(Temp[0], Mask), 4));
        Temp[0] = _mm256_srli_epi32(Temp[0], 8);
        Temp[1] = _mm256_xor_si256(
            Temp[1],
            _mm256_i32gather_epi32((const int *)BOX2,
                                   _mm256_and_si256(Temp[0], Mask), 4));
        Temp[0] = _mm256_srli_epi32(Temp[0], 8);
        Temp[1] = _mm256_xor_si256(
            Temp[1],
            _mm256_i32gather_epi32((const int *)BOX3,
                                   _mm256_and_si256(Temp[0], Mask), 4));
    ...
    /* Byte-swap and unpack back to 8 sequential blocks */
    X[0] = _mm256_shuffle_epi8(X[0], vindex);
    X[1] = _mm256_shuffle_epi8(X[1], vindex);
    X[2] = _mm256_shuffle_epi8(X[2], vindex);
    X[3] = _mm256_shuffle_epi8(X[3], vindex);
...
static void sm4_simd_encrypt_x8(const uint8_t *in, uint8_t *out,
                                const uint32_t rk[32])
{
    sm4_simd_do(in, out, rk, 0);
}

/* ========== Multibuffer CBC wrapper (8 streams, lockstep SIMD) ========== */
...
/* CBC encrypt for 8 streams in lockstep using SIMD.
 * Each stream processes DATA_LEN bytes.
 * plain / cipher are laid out: stream0[DATA_LEN], stream1[DATA_LEN], ... */
static void simd_cbc_encrypt_multibuffer(const uint8_t *plain,
                                          uint8_t *cipher,
                                          size_t len_per_stream,
                                          const uint32_t rk[32],
                                          uint8_t base_iv[SM4_BLOCK_SIZE])
{
    uint8_t ivs[NUM_STREAMS][SM4_BLOCK_SIZE];
    size_t pos_stream[NUM_STREAMS];

    /* Initialize per-stream IV (differentiate by stream id) */
    for (int s = 0; s < NUM_STREAMS; s++) {
        memcpy(ivs[s], base_iv, SM4_BLOCK_SIZE);
        ivs[s][15] ^= (uint8_t)s;
        pos_stream[s] = (size_t)s * len_per_stream;
    }

    size_t num_blocks = len_per_stream / SM4_BLOCK_SIZE;

    for (size_t b = 0; b < num_blocks; b++) {
        uint8_t batch_in[NUM_STREAMS * SM4_BLOCK_SIZE];
        uint8_t batch_out[NUM_STREAMS * SM4_BLOCK_SIZE];

        /* Gather one block from each stream and XOR with IV */
        for (int s = 0; s < NUM_STREAMS; s++) {
            for (int j = 0; j < SM4_BLOCK_SIZE; j++) {
                batch_in[s * SM4_BLOCK_SIZE + j] =
                    plain[pos_stream[s] + j] ^ ivs[s][j];
            }
        }

        /* SIMD encrypt all 8 blocks at once */
        sm4_simd_encrypt_x8(batch_in, batch_out, rk);

        /* Scatter back to cipher buffers and update IVs */
        for (int s = 0; s < NUM_STREAMS; s++) {
            for (int j = 0; j < SM4_BLOCK_SIZE; j++) {
                cipher[pos_stream[s] + j] = batch_out[s * SM4_BLOCK_SIZE + j];
            }
            memcpy(ivs[s], batch_out + s * SM4_BLOCK_SIZE, SM4_BLOCK_SIZE);
            pos_stream[s] += SM4_BLOCK_SIZE;
        }
    }
}
...
int main(void)
{
    ...
    uint32_t rk[32];
    size_t total_bytes = (size_t)NUM_STREAMS * DATA_LEN;
    ...
    sm4_simd_key_init(key, rk);

    double start = now_seconds();
    for (int i = 0; i < LOOP; i++) {
        simd_cbc_encrypt_multibuffer(plain, cipher, DATA_LEN, rk, (uint8_t *)base_iv);
    }
    double elapsed = now_seconds() - start;

    uint32_t cs = sm4_checksum(cipher, total_bytes);

    double total_data = (double)NUM_STREAMS * (double)LOOP * (double)DATA_LEN;
    printf("version: multi-buffer-simd+bitslice-sbox\n");
    printf("threads: %d\n", NUM_STREAMS);
